...
const QuantityStepper: React.FC<{ productId: string }> = ({ productId }) => {
  ...
  return (
    <div onClick={(e) => e.stopPropagation()} className="flex items-center justify-center w-full h-8 bg-blinkit-green-light rounded-lg">
      <button
      ...
      <span className="px-3 text-sm font-bold text-blinkit-green">{quantity}</span>
      <button
        onClick={() => updateQuantity(productId, quantity + 1)}
...
const AddButton: React.FC<{ product: Product }> = ({ product }) => {
  ...
  return (
    <button
      ...
      className="absolute bottom-2 -right-2 bg-white shadow-md border border-gray-200 text-blinkit-green text-xs font-bold px-4 py-1.5 rounded-lg hover:bg-blinkit-green-light transition-colors"
    >
      ADD
    </button>
  );
